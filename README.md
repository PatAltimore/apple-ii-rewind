...
- **Desktop-only extras**: **⛶ Fullscreen** puts the game view in true chromeless fullscreen, letterboxed to keep its aspect ratio (Escape — the browser's own built-in gesture — leaves it; <kbd>F2</kbd>/<kbd>Backspace</kbd> still rewind while fullscreen even without the visible bar). **] Applesoft** breaks straight into an Applesoft BASIC prompt, hiding the game UI while active; this is a preview — the prompt renders and echoes keystrokes correctly, but submitting a line with Return isn't fully reliable yet. <kbd>Delete</kbd> (Ctrl-Reset) always returns to Total Replay.
