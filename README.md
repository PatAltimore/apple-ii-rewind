...
- **Loading other disks**: the **Disk** row under the game has a short list of Internet Archive titles (Lemonade Stand, the DOS 3.3 System Master, The Oregon Trail, …) and a box to paste any archive.org disk link — the "download" URL for a `.dsk` / `.do` / `.po` / `.woz` / `.hdv` / `.2mg` file, or any other CORS-enabled URL. Choosing one reloads the page with a `?disk=<url>` parameter (so the link is shareable and every boot uses the same cold-boot path); the image is fetched through `cors.archive.org`, browser-cached like the Total Replay image, and mounted in place of Total Replay (5.25″ images get a Disk II card in slot 6; hard-drive images use the SmartPort card). Nothing is bundled — the list in `web/public/disks/library.json` is just pointers, which you can edit. **⏏ Total Replay** (or reloading without `?disk=`) goes back.
