...
- USB/Bluetooth gamepads are picked up by apple2js's Gamepad API support: the left stick acts as the joystick and **A**/**B**/left-bumper/right-bumper are the two fire buttons, for games. The **D-pad navigates the Total Replay menu** (same arrow-key codes the keyboard sends — the menu doesn't read the joystick), **Start launches** the highlighted game, **Back** sends Esc, and **X rewinds 5 seconds** (same as ⏪ -5s/<kbd>F2</kbd>).
- **Desktop-only**: **⛶ Fullscreen** puts the game view in true chromeless fullscreen, scaled to the largest size that fits your screen at the game's native aspect ratio (Escape — the browser's own built-in gesture — leaves it; <kbd>F2</kbd>/<kbd>Backspace</kbd> still rewind while fullscreen even without the visible bar).
- **Collapsible sections** below the rewind bar — touch controls, then the keyboard-shortcut legend, then disk switching — each a native, no-JS `<details>` panel, expanded by default; click a header to tuck it away. Which of touch/keyboard shows by default still follows the device auto-detect (or your remembered "Switch to touch/keyboard controls" choice) — see Controls below.
...
- **Loading other disks**: the **💾 Disk image** section has **] Applesoft** (leaves Total Replay for a freshly cold-booted, fully interactive Applesoft BASIC prompt — no disk mounted, same as a real Apple II with nothing to boot from), a short list of Internet Archive titles (the DOS 3.3 System Master, The Oregon Trail, …), a box to paste any archive.org disk link — the "download" URL for a `.dsk` / `.do` / `.po` / `.woz` / `.hdv` / `.2mg` file, or any other CORS-enabled URL — a **Find** button that opens the [4am Apple II preservation collection](https://archive.org/details/apple_ii_library_4am) in a new tab, and **⏏ Total Replay** to go back. Choosing a disk reloads the page with a `?disk=<url>` parameter (so the link is shareable and every boot uses the same cold-boot path); the image is fetched through `cors.archive.org`, browser-cached like the Total Replay image, and mounted in place of Total Replay (5.25″ images get a Disk II card in slot 6; hard-drive images use the SmartPort card). Nothing is bundled — the list in `web/public/disks/library.json` is just pointers, which you can edit.
...
| "Load another disk" list | [various archive.org items](https://archive.org/details/apple_ii_library_4am) | belong to their respective publishers | `web/public/disks/library.json` holds only titles + URLs. Images are fetched at runtime through `cors.archive.org` on user action, cached in the browser, never stored in or served from this repo. |
