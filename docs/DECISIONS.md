...
## 2026-09-08 — Applesoft break-in: shipped with a known limitation on submitting a line

`breakToApplesoft()` (`EmulatorController.ts`) jumps the CPU straight to Applesoft's `RESTART` entry ($E003 → $D43C, confirmed by reading the ROM's own bytes — see the function's comment) rather than doing a real cold boot, so it's instant and doesn't disturb the mounted drive. Getting a *visible, working* prompt needed three fixes beyond the bare jump, each confirmed against this exact ROM by reading memory/registers live rather than by reference-doc addresses (which were wrong twice during this work — see the function's comments):

1. Total Replay leaves the text window restricted to a few bottom rows and the cursor off past row 23; `RESTART`'s own screen-clear-and-prompt ran correctly but entirely outside the visible screen. Fixed by resetting the standard zero-page window/cursor variables and TEXT/PAGE1 softswitches before jumping in.
2. Any access into an empty peripheral slot's ROM space (apple2js's accurate emulation of the //e's real "touching $C100-$C7FF banks in $C800 expansion ROM" hardware behavior) left the CPU spinning forever once something in Applesoft's startup path touched one. Fixed with `EmptySlotStub`, an RTS-for-everything card installed in slots 1-6 (Total Replay only uses 7).

With both fixes, the prompt renders correctly and keystrokes echo correctly (verified via screenshot and by reading text-page memory directly). **Known limitation**: pressing Return to submit a line reliably drops into the machine-language monitor instead of executing it — confirmed independent of both fixes above (it reproduces on a bare jump with no patches at all), and confirmed *not* to reproduce when reached via a real, full cold boot (all 7 slots stubbed, reset through the hardware vector, no direct jump) instead — meaning `RESTART` needs some further piece of state a genuine cold boot sets up that a warm jump doesn't (most likely BASIC's own program/variable-pointer zero-page block — $67 `TXTTAB` onward — though attempts to read correct values for those empirically, the same way the window/cursor fix's values were sourced, did not resolve it within the time available). Ctrl-Reset (Delete) reliably recovers from this every time, matching real hardware's own general answer to a wedged machine, so the feature ships as "see and explore a real Applesoft prompt" rather than "fully interactive BASIC session" for now. Revisiting this — likely by finding the correct stack/pointer state a genuine cold boot leaves before reaching `RESTART`, or by running the real cold-start routine for real (`cpu.stepCycles`) and layering just the window/cursor fix on top without re-clearing what it already drew — is a good follow-up.
